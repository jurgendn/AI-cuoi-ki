...
class cSE(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.pointwise_1 = nn.Conv2d(in_channels=in_channels,
...
class DSConvSE(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.bn(x)
        x = self.act(x)
        x = torch.add(self.sSE(x), self.cSE(x))
        return x
...
class S2Block(nn.Module):
    ...
    def __init__(self,
        ...
        self.dconv = nn.Conv2d(n_channels,
                               n_channels,
                               kernel_size=kernel_size,
                               padding=pad,
                               groups=n_channels,
        ...
        self.pwconv = nn.Conv2d(n_channels,
                                n_channels,
                                kernel_size=1,
                                bias=False)
...
class SINetEncoder(nn.Module):
    ...
    def __init__(self, in_channels, n_classes):
        super().__init__()
        with open("config/SegModel20.json", "r") as f:
            s2_modules_config = json.load(f)['model']["s2Module"]
        ...
        for i in range(len(s2_modules_config)):
            ...
            setattr(
                self, module_name,
                S2Module(_in_channels, _out_channels, s2_block_1_args,
                         s2_block_2_args))
...
class SINetDecoder(nn.Module):
    ...
    def __init__(self, in_channels, inf_block_channels):
        ...
        self.inf_block_conv = nn.Conv2d(inf_block_channels,
                                        in_channels,
                                        kernel_size=3,
                                        padding=1,
                                        bias=False)
